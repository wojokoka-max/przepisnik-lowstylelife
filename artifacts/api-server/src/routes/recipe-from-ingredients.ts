...
Użytkownik podaje LISTĘ składników, które ma w lodówce/szafce (np. "jajka, musztarda, natka pietruszki").
Twoim zadaniem jest wymyślić jeden konkretny, realistyczny domowy przepis, który wykorzystuje przede wszystkim podane składniki.

Zasady:
- Używaj GŁÓWNIE podanych składników. Możesz założyć, że użytkownik ma podstawowe produkty spiżarniane (sól, pieprz, olej/oliwa, woda, masło) i dodać je w razie potrzeby.
- Nie wymyślaj wielu egzotycznych dodatków — przepis ma być możliwy do zrobienia z tego, co użytkownik wymienił.
- Nadaj daniu apetyczną, konkretną nazwę.
...
Format JSON:
{
  "title": "string — apetyczna nazwa dania",
  ...
  "ingredients": ["string", ...] — każdy składnik z ilością, np. "3 jajka",
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const rawList: string[] = Array.isArray(body.ingredients)
      ? body.ingredients.filter((x): x is string => typeof x === "string")
    ...
    if (ingredients.length === 0) {
      res.status(400).json({ error: "Podaj przynajmniej jeden składnik." });
      return;
    }
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: `Składniki w lodówce: ${ingredients.join(", ")}` },
      ],
      max_tokens: 1500,
    });
    ...
    res.json({
      title:       safeStr(parsed.title, "Danie z lodówki"),
    ...
    console.error("[recipe-from-ingredients]", err);
