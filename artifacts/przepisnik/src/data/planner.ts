...
const BASE_DIET_PLANS: Record<Exclude<DietName, 'szybkie_20' | 'bezglutenowa'>, Record<MealSlot, number[]>> = {
  low_carb: {
    ...
    podwieczorek:     [40, 81, 34, 166, 175, 177, 197, 182, 190, 181, 188, 189, 376, 379, 381, 384, 385, 386, 389, 391, 473, 476, 478, 479, 483, 486, 487, 489, 490],
  ...
  weganska: {
    ...
    podwieczorek:     [83, 78, 80, 167, 172, 176, 179, 184, 193, 162, 165, 378, 380, 385, 388, 392, 393, 396, 398, 475, 480, 481, 484, 389, 495, 498],
  ...
  wegetarianska: {
    ...
    podwieczorek:     [83, 40, 32, 172, 170, 184, 175, 193, 162, 165, 179, 376, 378, 380, 384, 388, 392, 396, 398, 473, 475, 476, 477, 478, 480, 481, 484, 487, 490, 493, 495, 498],
  ...
  niskotluszczowa: {
    ...
    podwieczorek:     [32, 33, 81, 171, 176, 187, 182, 191, 199, 206, 232, 388, 392, 396, 398, 475, 480, 481, 484, 487, 390, 490, 493, 495],
...
const TYPE_PATTERNS: [DishType, RegExp][] = [
  ...
  ['deser',       /^sernik|^ciasto|^mus\b|^pianka|^lody|^brownie|^tarta|^naleśnik|^gofr|^pancakes|^placki|panna cotta|^krem\s/i],
...
export function pickDiverseTriple(
  ...
  const byType = new Map<DishType, number[]>()
  for (const id of working) {
    const t = DISH_TYPE_MAP.get(id) ?? 'inne'
  ...
  // Rotacja typów wg seeda — dzięki temu każdego dnia inny zestaw kategorii
  const offset = ((seed % types.length) + types.length) % types.length
  const rotated = [...types.slice(offset), ...types.slice(0, offset)]

  const result: number[] = []
  for (let i = 0; i < rotated.length && result.length < 3; i++) {
    const dishes = byType.get(rotated[i])!
    const idx = ((seed * (i + 7)) % dishes.length + dishes.length) % dishes.length
    result.push(dishes[idx])
  }

  // Jeśli kategorii było mniej niż 3, dopełnij z reszty puli roboczej
  if (result.length < 3) {
    const remaining = working.filter(id => !result.includes(id))
    let s = seed | 0
    while (result.length < 3 && remaining.length > 0) {
      s = (s * 31 + 13) | 0
      const idx = ((s % remaining.length) + remaining.length) % remaining.length
