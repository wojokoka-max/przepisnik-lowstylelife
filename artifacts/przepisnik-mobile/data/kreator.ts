...
/**
 * Wybiera przepis dopasowany do produktu.
 *
 * Strategia:
 * 1. Najpierw przepisy ściśle dopasowane do produktu (`r.match(p)`).
 * 2. Jeśli to za mało, dokładamy przepisy pasujące do dowolnego
 *    produktu z tej samej kategorii — pozwala to "Inny wariant"
 *    pokazać zupełnie inną technikę (np. dla tuńczyka mamy tylko
 *    sałatkę, ale dla ryb w kategorii istnieje też wersja z patelni).
 * 3. `seen` pamięta wszystkie pokazane przepisy w aktualnej sesji
 *    składnika — jeśli pula się wyczerpie, resetujemy i lecimy od nowa.
 */
export function findRecipe(
  p: Product,
  seen: Set<string>
): KreatorRecipe {
  const exact = RECIPES.filter((r) => r.id !== "fallback" && r.match(p));
  const byCategory = RECIPES.filter(
    (r) =>
      r.id !== "fallback" &&
      !exact.includes(r) &&
      DB.some((q) => q.cat === p.cat && r.match(q)),
  );
  const allOptions = [...exact, ...byCategory];
  ...
  let pool = allOptions.filter((r) => !seen.has(r.id));
  if (!pool.length) {
    // wyczerpaliśmy wszystkie warianty — resetujemy historię
    seen.clear();
    pool = allOptions;
  }
